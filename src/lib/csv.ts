...
export function leadsToCsv(leads: Lead[]): string {
  ...
  const rows = leads.map((lead) => HEADERS.map((h) => escapeCsvValue(lead[h.key] ?? '')).join(','));
...
export function parseCsvToLeads(csvText: string): Lead[] {
  ...
  const columnMap: Array<keyof Lead | null> = headerRow.map((header) => {
    ...
    return HEADER_ALIASES[header] || HEADER_ALIASES[compact] || null;
  });

  if (!columnMap.includes('businessName')) {
    throw new Error('CSV must include a "Business Name" column (or "Name" / "Business").');
  }
